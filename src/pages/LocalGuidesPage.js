...
const LocalGuidesPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter data untuk hanya menampilkan local guides
      const localGuidesData = data.filter(item => item.category === 'local guides');
      setGuides(localGuidesData);
      
      // Get unique categories
      const uniqueCategories = ['All', ...new Set(localGuidesData.map(item => item.category).filter(Boolean))];
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Filter hasil pencarian untuk local guides
      const localGuidesResults = searchResults.filter(item => item.category === 'local guides');
      setGuides(localGuidesResults);
      setError(null);
    } catch (err) {
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <input 
              type="text"
              placeholder="Cari local guide yang Anda inginkan..."
              value={searchTerm}
            ...
            {guides.length === 0 ? (
              <div style={{
                ...
                <h3 style={{ 
                  ...
                  marginBottom: '12px',
                  color: '#333'
                }}>Tidak ada local guide ditemukan</h3>
                <p style={{ 
                  fontSize: '14px',
                  lineHeight: '1.5',
                  maxWidth: '400px',
              ...
              <div style={{
                ...
                {guides.map(guide => (
                  <div 
                    key={guide.id}
                    onClick={() => window.location.href = `/product/${guide.id}`}
                    style={{
                      background: 'white',
                      borderRadius: '16px',
                      overflow: 'hidden',
                  ...
                  >
                    ...
                    <img 
                      src={guide.image_url || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80'}
                      alt={guide.name}
                      onError={(e) => {
